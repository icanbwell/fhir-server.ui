...
class RethrownError extends Error {
     ...
     * Constructor
     */
    constructor({message, error, args, source}: { message?: string, error: Error, args?: object, source?: string }) {
        ...
        if (Array.isArray(args)) {
            for (let prop in args) {
                logExcludeResources.forEach(resource => {
                    if (args[prop] && args[prop].resourceType === resource) {
                        delete args[prop];
                    }
                });
                this.removeExcludedResources(args[prop]);
            }
        }
    }
     ...
     */
    buildStackDescriptor(oldStackDescriptor: PropertyDescriptor, nested: Error): { value: string } | { get: () => string } {
