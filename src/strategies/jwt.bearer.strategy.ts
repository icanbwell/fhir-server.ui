...
let openIdClientIssuer: Issuer<BaseClient> | null = null;
...
function parseUserInfoFromPayload({
    ...
    console.log(`JwtBearerStrategy parseUserInfoFromPayload: ${JSON.stringify(jwt_payload)}`);
...
const verify = (
    ...
    console.log(`JwtBearerStrategy verify: ${JSON.stringify(jwt_payload)}`);
    if (jwt_payload) {
        let isUser = false;
            ...
            if (accessToken) {
                return getUserInfoAsync(accessToken)
                    .then((id_token_payload) => {
                        // @ts-ignore
                        const payload: Record<string, string> = id_token_payload;
