...
function CodeableConceptWithValue({
  name,
  ...
  if (
    !(codeableConcepts && codeableConcepts.length > 0 && codeableConcepts[0])
  ) {
    return null;
  }
  ...
  const handleCodingSystem = (coding) => {
    switch (coding.system) {
      ...
      case 'http://hl7.org/fhir/sid/icd-10':
        const code =
          coding.code.includes('.') || coding.code.length < 4
            ? coding.code
            : coding.code.substr(0, 3) + '.' + coding.code.substr(3, 10);
        ...
        <Table className="table">
          ...
          <TableBody>
            {codeableConcepts.map((concept) => {
              if (
                concept &&
                concept[codePropertyName] &&
                concept[codePropertyName].coding
              ) {
                return concept[codePropertyName].coding.map((coding, i) => {
                  if (coding) {
                    ...
                    const system = searchParameter ? (
                      <Link href={systemUrl}>{coding.system}</Link>
                    ) : (
                      coding.system
                    );
                    const code = searchParameter ? (
                      <Link href={codeUrl}>{coding.code}</Link>
                    ) : (
                      <Link href={handleCodingSystem(coding)}>
                        {coding.code}
                      </Link>
                    );
