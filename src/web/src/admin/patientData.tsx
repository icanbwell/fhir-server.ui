...
const PatientDataPage: React.FC = () => {
    ...
    const handlePatientDataSubmit = async (event: React.FormEvent) => {
        ...
        await new AdminApi(fhirUrl).getEverythingForPatient(patientId);
    };

    const handleDeletePatientDataSubmit = async (event: React.FormEvent) => {
        event.preventDefault();
        await new AdminApi(fhirUrl).deletePatient(patientId);
    };

    const handlePersonDataSubmit = async (event: React.FormEvent) => {
        event.preventDefault();
        await new AdminApi(fhirUrl).getEverythingForPerson(personId);
    };
    ...
    return (
        <Container maxWidth="sm">
            ...
            <Typography variant="h6">Deletes the graph of patient data (CONFIRM YOU ARE DOING THIS FOR THE CORRECT
                PATIENT)</Typography>
            ...
            <Typography variant="h6">Deletes the graph of person data (CONFIRM YOU ARE DOING THIS FOR THE CORRECT
                PERSON)</Typography>
